...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($statusClass === 'active' && !isDonorDisabled($row['id'])) {
        ...
        echo '<form method="POST" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '">';
        echo '<input type="hidden" name="donorId" value="' . $row['id'] . '">';
        echo '<button type="submit" class="disable-button" name="deleteButton">Delete</button>';
        echo '</form>';
...
function deleteDonor($donorId) {
    global $conn;
    $deleteQuery = "DELETE FROM donar WHERE id = '$donorId'";
...
<style>
    ...
    /* Additional CSS to hide active status toggle for disabled donors */
...
<script>
    ...
    function disableDonor(donorId, email) {
        ...
        var disableButton = document.getElementById("disable-button-" + donorId);
        disableButton.style.display = "none";

        var disabledMessage = document.getElementById("disabled-message-" + donorId);
        disabledMessage.style.display = "block";

        // Uncomment the code below to open the user's default email client
